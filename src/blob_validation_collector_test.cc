...
class BlobValidationCollectorTest : public testing::Test {
 public:
...
TEST_F(BlobValidationCollectorTest, Basic) {
  ...
  auto iter = table_properties->user_collected_properties.find(
      BlobValidationCollector::kPropertiesName);
  ASSERT_TRUE(iter != table_properties->user_collected_properties.end());

  Slice raw_blob_validation_prop(iter->second);
  std::unordered_map<uint64_t, std::vector<uint64_t>> result;
  BlobValidationCollector::Decode(&raw_blob_validation_prop, &result);

  ASSERT_EQ(2, result.size());

  ASSERT_EQ(kNumEntries / 2, result[kFirstFileNumber].size());
  ASSERT_EQ(kNumEntries / 2, result[kSecondFileNumber].size());
}
